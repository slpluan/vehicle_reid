...
def eval_func(distmat, q_pids, g_pids, q_camids, g_camids, max_rank=50):
    ...
    for q_idx in range(num_q):
        ...
        q_pid = q_pids[q_idx] #veri
        q_camid = q_camids[q_idx] #veri

        # remove gallery samples that have the same pid and camid with query
        order = indices[q_idx]  # select one row #veri
        remove = (g_pids[order] == q_pid) & (g_camids[order] == q_camid) #veri
        # remove = False #vehicleid
