...
def train_collate_fn(batch):
    ...
    """
    imgs, pids, _, _, = zip(*batch)#zip()将打包为元组的列表解压，返回原来的形式
    pids = torch.tensor(pids, dtype=torch.int64)
    return torch.stack(imgs, dim=0), pids#stack()沿着一个新维度对输入张量序列进行拼接，序列中所有张量应为相同形状
...
def make_dataloader(cfg):
    train_transforms = T.Compose([
        ...
        T.RandomHorizontalFlip(p=0.5),#依概率p对PIL图片进行水平翻转
        T.Pad(10),#图像填充
        ...
        RandomErasing(probability=0.5, sh=0.4, mean=(0.485, 0.456, 0.406))#数据增强之随即擦除
    ])
    ...
    dataset = VehicleID(data_dir=cfg.DATA_DIR, verbose=True)#verbose日志显示
